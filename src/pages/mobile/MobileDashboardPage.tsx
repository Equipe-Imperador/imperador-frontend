// src/pages/mobile/MobileDashboardPage.tsx
...
export default function MobileDashboardPage() {
  ...
  const handleSensorToggle = (sensorId: string) => {
    setVisibleSensors(prev => prev.includes(sensorId) ? prev.filter(id => id !== sensorId) : [...prev, sensorId]);
  };
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: '#131E33', color: '#ccc', p: 2 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
          {role === 'integrante' && (
            <Button variant="outlined" sx={{ color: '#ccc', borderColor: '#ccc' }} onClick={() => navigate('/mobile/export')}>Exportar</Button>
          )}
          ...
          <>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 1 }}>
              ...
              <input
                ...
                style={{ padding: 8, borderRadius: 6, border: '1px solid #444', background:'#0b1a2a', color:'#fff' }}
              />
              ...
              <Button fullWidth variant="outlined" onClick={handleLast10Minutes} sx={{ color: '#ccc', borderColor: '#ccc' }}>
                Últimos 10 Minutos
              </Button>
            </Box>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          {sensorConfig.map(sensor => (
            <Button
              ...
              sx={{ bgcolor: visibleSensors.includes(sensor.id) ? '#00142F' : 'transparent', color: '#ccc', borderColor: '#444', fontSize: 12 }}
            >
              {sensor.label}
            </Button>
          ))}
      ...
      {/* Gauges */}
      <Box sx={{ display: 'flex', gap: 6 , overflowX: 'auto', mb: 2 }}>
        {widgetsToShow.map(widget => (
          <Box key={widget.id} sx={{ minWidth: 140 }}>
            <GaugeComponent
              label={widget.label}
      ...
      <Box>
        ...
        {widgetsToShow.map((chart, idx) => (
          <Box key={chart.id} sx={{ width: 'auto', height: idx === widgetsToShow.length - 1 ? 220 : 180, mb: 2 }}>
            <ResponsiveContainer>
              <LineChart data={historicalData}>
                <CartesianGrid strokeDasharray="3 3" stroke="#444" />
                <XAxis dataKey="time" tickFormatter={(t) => new Date(t).toLocaleTimeString()} stroke="#ccc" />
                <YAxis stroke="#ccc" />
